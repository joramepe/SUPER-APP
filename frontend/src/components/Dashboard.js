...
const Dashboard = ({ players, tournaments, matches, refreshData }) => {
  ...
  const [weeksAtNumber1, setWeeksAtNumber1] = useState({});
              ...
              {weeksAtNumber1.total_weeks && Object.keys(weeksAtNumber1.total_weeks).length > 0 && (
                <div className="border-t pt-4">
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="text-xs font-medium text-gray-600 mb-1">Total Semanas:</div>
                  ...
                  {weeksAtNumber1.longest_streak && (
                    <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg p-3 mb-4 border border-yellow-200">
                      <div className="text-xs font-medium text-gray-600 mb-1">Récord Consecutivo:</div>
                      <div className="flex items-center justify-between">
                        <div className="font-bold text-yellow-800">
                          🏆 {weeksAtNumber1.longest_streak.player_name}
                        </div>
                        <div className="text-right">
                          <div className="font-bold text-yellow-700">
                            {weeksAtNumber1.longest_streak.weeks} semanas
                          </div>
                          <div className="text-xs text-yellow-600">consecutivas</div>
                        </div>
                      </div>
                    </div>
                  )}

                  {/* All consecutive streaks */}
                  {weeksAtNumber1.consecutive_streaks && (
                    <div className="mb-4">
                      <div className="text-xs font-medium text-gray-600 mb-1">Máximo Consecutivo por Jugador:</div>
                      <div className="space-y-1">
                        {Object.entries(weeksAtNumber1.consecutive_streaks)
                          .sort((a, b) => b[1].max_consecutive_weeks - a[1].max_consecutive_weeks)
                          .map(([playerName, data]) => (
                            <div 
                              key={playerName} 
                            ...
                            >
                              ...
                              <span className={`font-medium ${
                                data.max_consecutive_weeks > 0 ? 'text-orange-600' : 'text-gray-500'
                              }`}>
                                {data.max_consecutive_weeks} sem.
                              </span>
                            </div>
                          ))}
                      </div>
